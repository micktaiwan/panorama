...
import '/imports/api/opportunities/collections';
import '/imports/api/opportunities/publications';
import '/imports/api/opportunities/methods';
import '/imports/api/staffing/collections';
import '/imports/api/staffing/publications';
import '/imports/api/staffing/methods';
import '/imports/api/staffing/gitCollections';
import '/imports/api/staffing/gitPublications';
import '/imports/api/staffing/gitMethods';
...
Meteor.startup(async () => {
  ...
  const { OpportunitiesCollection } = await import('/imports/api/opportunities/collections');
  const { StaffingCollection } = await import('/imports/api/staffing/collections');
  const { CommitsCollection, BranchClassificationsCollection, OpportunitySuggestionsCollection } = await import('/imports/api/staffing/gitCollections');
  ...
  StaffingCollection.rawCollection().createIndex({ userId: 1, personId: 1 }).catch(() => {});
  CommitsCollection.rawCollection().createIndex({ userId: 1, sha: 1 }, { unique: true }).catch(() => {});
  CommitsCollection.rawCollection().createIndex({ userId: 1, committedAt: -1 }).catch(() => {});
  BranchClassificationsCollection.rawCollection().createIndex({ userId: 1, branch: 1 }, { unique: true }).catch(() => {});
  OpportunitySuggestionsCollection.rawCollection().createIndex({ userId: 1, nameNorm: 1 }, { unique: true }).catch(() => {});
