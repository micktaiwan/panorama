...
Meteor.methods({
  async 'situationActors.insert'(fields) {
    ...
    const now = new Date();
    const doc = { situationId, name, role, situationRole, userId: this.userId, createdAt: now, updatedAt: now };
  ...
  async 'situationActors.update'(id, fields) {
    check(id, String);
    ensureLoggedIn(this.userId);
    await ensureOwner(SituationActorsCollection, id, this.userId);
    if (!fields || typeof fields !== 'object') throw new Meteor.Error('invalid-arg', 'fields must be an object');
    const updates = {};
  ...
  async 'situationActors.remove'(id) {
    ...
    await SituationNotesCollection.removeAsync({ actorId: id, userId: this.userId });
