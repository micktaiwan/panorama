...
export const buildFilterSelector = (filters = {}) => {
  ...
  // Add dueBefore filtering (merged from buildTasksSelector)
  if (filters && typeof filters.dueBefore === 'string' && filters.dueBefore.trim()) {
    const dt = new Date(filters.dueBefore);
    if (!Number.isNaN(dt.getTime())) {
      // Match either a Date field or a string YYYY-MM-DD field
      const ymd = dt.toISOString().slice(0, 10);
      sel.$or = [
